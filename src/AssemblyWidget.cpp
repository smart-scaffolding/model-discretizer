...
void AssemblyWidget::on_saveInstructionsButton_pressed()
{
  ...
  for (int level = 0; level < legoCloudNode->getLegoCloud()->getLevelNumber(); level++)
  {
    ...
    std::cout << "Size: " << imageSizeX << imageSizeY << std::endl;

    for (QList<LegoBrick>::const_iterator brickIt = legoCloudNode->getLegoCloud()->getBricks(level).begin(); brickIt != legoCloudNode->getLegoCloud()->getBricks(level).constEnd(); brickIt++)
    {
      const LegoBrick *brick = &(*brickIt);

      std::cout << "Brick: "
                << "\n\tSize: " << brick->getSizeX() << " " << brick->getSizeY() << "\n\tPostition: " << brick->getPosX() << " " << brick->getPosY() << std::endl;
    }
    if (useSVG)
    {
